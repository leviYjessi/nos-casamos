...
const Index = (): JSX.Element => {
  ...
  return (
    <div>
      <NavigationHeader />
      <div className="min-h-[calc(100vh-112px)] max-w-5xl m-auto bg-grey-bg">
        <div className="lg:pt-14">
          <HeaderImage
            imageSrc={`${process.env.NEXT_PUBLIC_IMAGES_ROUTE}images/date-rings-large.svg`}
        ...
        <form
          className="flex bg-grey-bg flex-col m-auto pt-10 px-6 max-w-5xl lg:px-44"
          method="post"
          onSubmit={handleOnSubmit}
        >
          <h2 className="tracking-widest leading-6">DÉJANOS TUS DATOS</h2>
          <label className=" text-black pt-4" htmlFor="name">
            Nombre y apellidos
          </label>
          <input
            className="bg-light-grey border border-primary-grey  italic p-3 mt-2"
            type="text"
            id="name"
            name="name"
            required
          ...
          />
          <fieldset>
            <legend className=" text-black pt-8">
            ...
            <div className="pt-4">
              ...
              />
              <label className=" pl-2" htmlFor="isComing">
                ¡Por supuesto! 🤩
              </label>
            </div>
            <div className="pt-4">
              <input
                ...
                onClick={handleIsComingChange}
              />
              <label className=" pl-2" htmlFor="isNotComing">
            ...
            <>
              <fieldset>
                ...
                <div className="pt-4">
                  ...
                  />
                  <label className=" pl-2" htmlFor="hasCompanions">
                    Sí
                  </label>
                </div>
                <div>
                  <input
                    ...
                    onClick={handleHasCompanionsChange}
                  />
                  <label className=" pl-2" htmlFor="doesNotHaveCompanions">
                    No
                  </label>
                </div>
              </fieldset>
              {hasCompanions && (
                <>
                  <label
                    className=" text-black pt-8"
                    htmlFor="companionsNumber"
                  >
                    ¿Cuánta gente vendra contigo?
                  </label>
                  <select
                    className="bg-light-grey border border-primary-grey  italic p-3 mt-2"
                    id="companionsNumber"
                  ...
                  >
                    ...
                    {selectOptions.map((option) => (
                      <option key={`companionsNumber${option}`} value={option}>
                        {option}
                      </option>
                    ))}
                  </select>
                  {companionsArray.length > 1
                    ? companionsArray.map((companion, index) => (
                        <div className="pt-8" key={`companionName${index}`}>
                          <label
                            className=" text-black"
                            htmlFor={`companionName${index}`}
                          >
                            Nombre y apellidos del {companion} acompañante
                          </label>
                          <input
                            className="bg-light-grey w-full border border-primary-grey  italic p-3 mt-2"
                            type="text"
                            required={hasCompanions}
              ...
              <label className=" text-primary-black pt-8" htmlFor="suggestions">
                ¿TIENES ALGUNA PETICIÓN ESPECIAL?
              </label>
              <label className=" text-dark-grey pt-4" htmlFor="suggestions">
                ¿Tienes alergias? ¿Eres vegano? ¿Prefieres un menú infantil?...
          ...
          <div className="flex justify-end">
            <button
              className="py-3 px-10 mt-8 mb-12 font-bold tracking-widest  leading-6 bg-black text-white"
              type="submit"
              disabled={hasSubmit}
            >
              ENVIAR
